...
// Adapted from xenozauros's implementation of a hex to hsl function, from https://gist.github.com/xenozauros/f6e185c8de2a04cdfecf#file-hex2hsl-js
...
class StoredCalendarOpportunity {
	 ...
	 * @param {L.Marker|undefined} map_marker The map marker.
	 */
	constructor(element, opportunity, map_marker = undefined) {
		this._element = element;
		this._opportunity = opportunity;
		this._map_marker = map_marker;
	}
	 ...
	 * Gets the map marker.
	 * 
	 * @returns {L.Marker|undefined} The map marker.
	 */
	get map_marker() {
...
class Calendar {
	...
	constructor() {
		...
		// If the map frame does not exist, throw an error.
		if (!mapFrame) {
			throw new Error("Map frame not found!");
		}

		// Create the map.
		this._map = L.map(mapFrame);
		...
		let InitialView = L.latLng(43.946838185736134, -78.89790650351452);

		this._map.setView(InitialView, 15);
	...
	loadOpportunity(opportunity){
		...
		// If the opportunity date is not within our year, skip it.
		if (opportunityDate.getFullYear() !== this._date.getFullYear()) {
			// Print the opportunity date.
			console.log(`Skipping Opportunity: ${opportunity.title}, as it's year ${opportunityDate.getFullYear()} is not the same as the calendar's year ${this._date.getFullYear()}`);
			return;
		}
		...
		let columnIndex = dayOfTheMonth % Calendar.DAYS_PER_WEEK;

		// Get the row index for this day.
		let rowIndex = Math.floor(dayOfTheMonth / Calendar.DAYS_PER_WEEK);
		...
		// If the opportunity element has a latlng, then add it to the map.
		if(opportunity.latlng){
			// Create the marker.
			marker = L.marker(opportunity.latlng);

			// Add a popup for the marker, that shows the title and description when clicked.
			marker.bindPopup(`<b>${opportunity.title}</b><br>${opportunity.description}`);

			// Add the marker to the map.
			marker.addTo(this._map);

			this._updateMarker(opportunity.type, marker)
		}
	...
	setDate(date){
		...
		// Destroy all of the opportunities.
		for (let opportunityType in this._opportunities) {
			// Get the opportunities.
			let opportunities = this._opportunities[opportunityType];

			// Go through each opportunity.
			for (let opportunityIndex = 0; opportunityIndex < opportunities.length; opportunityIndex++) {
				// Get the opportunity.
				let opportunity = opportunities[opportunityIndex];

				// Remove the map marker.
				if(opportunity.map_marker){
					this._map.removeLayer(opportunity.map_marker);
				}

				// Remove the opportunity element.
				opportunity.element.remove();
			}
		}
		...
		for(let columnIndex = 0; columnIndex < Calendar.DAYS_PER_WEEK; columnIndex++) {
	...
	updateColumnWidths() {
		...
		for(let columnIndex = 0; columnIndex < Calendar.DAYS_PER_WEEK; columnIndex++) {
			...
			for(let rowIndex = 0; rowIndex < Calendar.CALENDAR_ROWS; rowIndex++) {
				...
				// If the cell does not exist, continue.
				if (!cell) {
					continue;
				}
	...
	/**
	 * Called to update the look of an opportunity map marker.
	 * 
	 * @param {OpportunityType} type The opportunity type.
	 * @param {L.Marker} marker The map marker.
	 */
	_updateMarker(type, marker){
		// Calculate how much to rotate the hue by.
			
		// Calculate the hue rotation of the colour blue.
		let baseHueRotation = hexToHSL("#2981ca").h*360;
		
		let hueRotation = hexToHSL(type.colour).h*360;

		let calculatedHue = hueRotation-baseHueRotation;

		// Print the hue rotations.
		//console.log(`Base Hue Rotation: ${baseHueRotation}, Hue Rotation: ${hueRotation} calculated hue: ${calculatedHue}`);	

		// Set the colour of the marker.
		marker.getElement().style.filter = `hue-rotate(${calculatedHue}deg)`;
	}
	...
	_updateOpportunityVisibility(storedOpportunity){
		...
		if(storedOpportunity.map_marker){
			if(checked){
				this._map.addLayer(storedOpportunity.map_marker);
				// Update the marker.
				this._updateMarker(storedOpportunity.opportunity.type, storedOpportunity.map_marker);
			}else{
				this._map.removeLayer(storedOpportunity.map_marker);
			}
		}
	}
	...
	_drawCalendarDays() {
		...
		for (let rowIndex = 0; rowIndex < Calendar.CALENDAR_ROWS; rowIndex++) {
			...
			for (let columnIndex = 0; columnIndex < Calendar.DAYS_PER_WEEK; columnIndex++) {
				...
				let dayOfTheMonth = rowIndex * Calendar.DAYS_PER_WEEK + columnIndex - firstDayWeekIndex + 1;
